...
export class BumpNotifySelectHandler extends InteractionHandler {
  public override parse(ctx: ChannelSelectMenuInteraction<'cached'>) {
    if (ctx.customId !== 'settings_bumps_notifyChannelId') return this.none();
  ...
  public override async run(ctx: ChannelSelectMenuInteraction<'cached'>) {
    ...
    await Settings.findOneAndUpdate({ guildId: ctx.guildId }, { 'bumps.notifyChannelId': channel.id }, { upsert: true, new: true });

    await ctx.update({
      embeds: [
        generateEmbed({ color: Colors.Green, title: '✅ | Налаштування збережено', description: `Ви змінили канал для нагадування.` }),
      ],
      components: [],
    });
  }
